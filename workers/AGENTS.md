...
- **Mailbox-local agent settings live in MailboxDO** after the PR 5/6 cutover. The `mailbox_settings` singleton row owns `autoDraft`, `agentSystemPrompt`, `invoiceAgentSystemPrompt`, model overrides (`agentModel`, `emailReplyModel`, `invoiceModel`), enabled-skill allowlists, and `invoiceSourceDomains`. The R2 settings blob retains only ACL fields and the legacy non-agent UI fields (`fromName`, `forwarding`, `signature`, `autoReply`) plus rules. `getAgentConfig` reads DO-first; un-backfilled legacy mailboxes self-heal from R2 on first read. `updateAgentConfig` and the `PUT /api/v1/mailboxes/:id` settings endpoint write the agent-config slice through the DO exclusively.

...
| `agent-config.ts` | Per-mailbox agent config (loaded from R2 settings blob) — `ALLOWED_AGENT_MODELS`, `DEFAULT_AGENT_MODEL`, `getAgentConfig` returning `{ autoDraft, model, customSystemPrompt, rules }`. `autoDraft` defaults true for legacy mailboxes |
